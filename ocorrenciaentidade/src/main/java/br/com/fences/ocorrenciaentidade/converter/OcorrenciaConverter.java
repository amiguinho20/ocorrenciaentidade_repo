...
//				.registerTypeHierarchyAdapter(Ocorrencia.class, new OcorrenciaJsonAdapter())
//				.create();
//		gsonComTransient = new GsonBuilder()
...
//			    .create();
	...
	@Override
	public Ocorrencia paraObjeto(Document doc, Class<Ocorrencia> clazz) {
		
		String jsonMongoDB = doc.toJson();
    	String json = transformarIdParaJsonObj(jsonMongoDB);
 		Ocorrencia ocorrencia = gson.fromJson(json, clazz);
    	...
    	if (doc.get("AUXILIAR") != null)
    	{
    		...
    		{
    			List<?> filhosDocList = (ArrayList<?>) docAuxiliar.get("FILHOS");
	...
	public String paraJson(Ocorrencia ocorrencia) {
		retirarReferenciaDosFilhosAoPai(ocorrencia);
		retirarReferenciaDoPaiAosFilhos(ocorrencia);
		return super.paraJson(ocorrencia);
	}
	
	@Override
	public String paraJson(Collection<Ocorrencia> ocorrencias) {
		...
		return super.paraJson(ocorrencias);
	}
	
	@Override
	public Ocorrencia paraObjeto(String json, Class<Ocorrencia> clazz) {
		Ocorrencia ocorrencia = super.paraObjeto(json, clazz);
		atribuirReferenciaDosFilhosAoPai(ocorrencia);
		atribuirReferenciaDoPaiAosFilhos(ocorrencia);
		return ocorrencia;
	}
	
	@Override
	public Collection<Ocorrencia> paraObjeto(String json, Type tipo) {
		Collection<Ocorrencia> ocorrencias = super.paraObjeto(json, tipo);
		if (Verificador.isValorado(ocorrencias))
		{
			for (Ocorrencia ocorrencia : ocorrencias)
			{
	...
	private void atribuirReferenciaDosFilhosAoPai(Ocorrencia ocorrencia)
	{
	...
	private void atribuirReferenciaDoPaiAosFilhos(Ocorrencia ocorrencia)
	{
		if (ocorrencia != null && ocorrencia.getAuxiliar() != null && ocorrencia.getAuxiliar().getPai() != null)
		{
			Ocorrencia pai = ocorrencia.getAuxiliar().getPai();
			if (pai.getAuxiliar() != null && pai.getAuxiliar().getFilhos() != null)
			{
				for (Ocorrencia filho : pai.getAuxiliar().getFilhos())
				{
					filho.getAuxiliar().setPai(pai);
				}
			}
		}
	}
	
	private void retirarReferenciaDosFilhosAoPai(Ocorrencia ocorrencia)
	{
		if (ocorrencia != null && ocorrencia.getAuxiliar() != null && ocorrencia.getAuxiliar().getFilhos() != null)
		{
			for (Ocorrencia filho : ocorrencia.getAuxiliar().getFilhos())
	...
	private void retirarReferenciaDoPaiAosFilhos(Ocorrencia ocorrencia)
	{
		if (ocorrencia != null && ocorrencia.getAuxiliar() != null && ocorrencia.getAuxiliar().getPai() != null)
		{
			Ocorrencia pai = ocorrencia.getAuxiliar().getPai();
			if (pai.getAuxiliar() != null && pai.getAuxiliar().getFilhos() != null)
			{
				pai.getAuxiliar().getFilhos().clear();
			}
		}
	}
